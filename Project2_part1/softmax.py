...
def update_y(train_y, test_y):
    ...
    """
    #YOUR CODE HERE
    updated_train_y= np.mod(train_y, 3)
    updated_test_y= np.mod(test_y, 3)
    return updated_train_y, updated_test_y
    raise NotImplementedError
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    y_class = get_classification(X, theta, temp_parameter)
    test_error = 1 - (np.mod(y_class, 3) == Y).mean()
    return test_error
