...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range (nsamples):
        feacture_vector=feature_matrix[i]
        prediction[i]= np.dot(feacture_vector, theta) + theta_0
        if prediction [i] > 0.0:
            prediction [i] =1
        else:
            prediction [i]= -1
...
def classifier_accuracy(
    ...
    """
    # Your code here
    (t_theta, t_theta_0)=classifier(train_feature_matrix, train_labels, **kwargs)
    t_prediction = classify(train_feature_matrix, t_theta, t_theta_0)
    v_prediction = classify(val_feature_matrix, t_theta, t_theta_0)

    t_accuracy= accuracy (t_prediction, train_labels)
    v_accuracy= accuracy (v_prediction, val_labels)
